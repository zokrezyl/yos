...
IM3Runtime  m3_NewSiblingRuntime  (IM3Runtime i_parent, u32 i_stackSizeInBytes, void * i_userdata)
{
    if (i_parent == NULL || i_parent->memory.mallocated == NULL)
        return NULL;

    IM3Runtime runtime = m3_NewRuntime (i_parent->environment, i_stackSizeInBytes, i_userdata);
    if (runtime == NULL)
        return NULL;

    // Adopt the parent's linear-memory buffer. The sibling will not allocate or free it,
    // and m3_LoadModule will skip InitMemory + InitDataSegments because of the shared flag.
    runtime->memory          = i_parent->memory;
    runtime->memory.shared   = true;
    runtime->memoryLimit     = i_parent->memoryLimit;
...
void  Runtime_Release  (IM3Runtime i_runtime)
{
    ...
    // Threads hack: only the owning runtime frees the linear-memory buffer.
    if (!i_runtime->memory.shared)
...
M3Result  InitMemory  (IM3Runtime io_runtime, IM3Module i_module)
{
    ...
    // Threads hack: a sibling runtime adopted its parent's linear memory.
    // Skip (re-)allocation; the parent already sized the buffer.
    if (io_runtime->memory.shared && io_runtime->memory.mallocated)
        return result;
...
M3Result  InitDataSegments  (M3Memory * io_memory, IM3Module io_module)
{
    ...
    for (u32 i = 0; i < io_module->numDataSegments; ++i)
    {
        ...
        // Passive segments have no init_expr; they are initialised at runtime
        // by the wasm `memory.init` opcode in the module's start function.
        if (segment->initExpr == NULL || segment->initExprSize == 0)
            continue;
...
_   (InitGlobals (io_module));
    // Threads hack: only the owning runtime initialises data segments. Siblings
    // skip this; the parent has already populated the shared buffer.
    if (!memory->shared)
