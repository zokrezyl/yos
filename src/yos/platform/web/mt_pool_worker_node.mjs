// Pooled thread worker (node worker_threads). Instantiates the module
// against the shared memory once, parks on its slot futex, runs assigned
// thread functions by table index. Mutex/cond/rwlock come from the shared
// Atomics in buildEnv; per-thread file I/O uses a worker-local VFS (each
// thread's temp file is self-contained, so no cross-worker sharing needed).
...
const { module, memory, slot } = workerData;
...
const ctx = {
  ...
  emit: (fd, t) => parentPort.postMessage({ out: t }),
  callMain: () => 0, callIndirect: (fn, a) => inst.exports.__indirect_function_table.get(fn)(a),
  write: (fd, p, n) => { const f = vfs.fds.get(fd); if (f && f.file) { const at = f.append ? f.file.data.length : f.off; const m = new Uint8Array(Math.max(f.file.data.length, at + n)); m.set(f.file.data); m.set(u8.subarray(p, p + n), at); f.file.data = m; f.off = at + n; return n; } parentPort.postMessage({ out: dec.decode(u8.slice(p, p + n)) }); return n; },
  ...
  access: () => -1, dupfd: () => vfs.nextFd++, opendir: () => 0, fdopendir: (fd) => fd, readdir: () => 0,
  sysctl: () => 0,
...
for (;;) {
  ...
  let rv = 0;
  try { rv = table.get(fnIdx)(arg) >>> 0; } catch (e) { if (e.isThreadExit) rv = e.code | 0; else parentPort.postMessage({ error: "thread: " + e.message }); }
