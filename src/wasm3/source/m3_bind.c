...
M3Result  LinkRawFunction  (IM3Module io_module,  IM3Function io_function, ccstr_t signature,  const void * i_function, const void * i_userdata)
{
    ...
    /* skip if already linked (allows specific links before wildcard) */
                                    ...
                                    voidptr_t       i_userdata)
{
    ...
    bool wildcardFunction = (strcmp (i_functionName, "*") == 0);
    ...
    for (u32 i = 0; i < io_module->numFunctions; ++i)
    {
        ...
        {
            if ((wildcardFunction or strcmp (f->import.fieldUtf8, i_functionName) == 0) and
               (wildcardModule or strcmp (f->import.moduleUtf8, i_moduleName) == 0))
            {
                ...
                if (wildcardFunction) {
                    result = m3Err_none; /* skip errors for wildcard */
