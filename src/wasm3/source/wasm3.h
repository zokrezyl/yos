...
//-------------------------------------------------------------------------------------------------------------------------------
    ...
    // Threads hack: create a runtime that shares linear memory with i_parent.
    // i_parent must already have a module loaded. Each sibling still parses & loads its
    // own copy of the wasm module (m3_ParseModule + m3_LoadModule); the load skips
    // memory allocation and data-segment init, reusing the parent's buffer instead.
    // Atomic ops on shared memory work across siblings; memory.grow is not supported.
    IM3Runtime          m3_NewSiblingRuntime        (IM3Runtime             i_parent,
                                                     uint32_t               i_stackSizeInBytes,
                                                     void *                 i_userdata);
