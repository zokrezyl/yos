...
// memory.init  data_idx  memory_idx     stack: [dst, src, size]
M3Result  Compile_Memory_Init  (IM3Compilation o, m3opcode_t i_opcode)
{
    M3Result result = m3Err_none;
    (void) i_opcode;
_try {
    u32 data_idx, mem_idx;
_   (ReadLEB_u32 (& data_idx, & o->wasm, o->wasmEnd));
_   (ReadLEB_u32 (& mem_idx,  & o->wasm, o->wasmEnd));   // must be 0 in MVP
    _throwif ("memory.init: invalid memory index", mem_idx != 0);
    _throwif ("memory.init: data index out of range",
              data_idx >= o->module->numDataSegments);

_   (CopyStackTopToRegister (o, false));   // size in r0
_   (EmitOp  (o, op_MemInit));
_   (PopType (o, c_m3Type_i32));
_   (EmitSlotNumOfStackTopAndPop (o));     // src
_   (EmitSlotNumOfStackTopAndPop (o));     // dst
    EmitPointer (o, & o->module->dataSegments [data_idx]);
}
    _catch: return result;
}

// data.drop  data_idx                     stack: []
M3Result  Compile_Memory_DataDrop  (IM3Compilation o, m3opcode_t i_opcode)
{
    M3Result result = m3Err_none;
    (void) i_opcode;
_try {
    u32 data_idx;
_   (ReadLEB_u32 (& data_idx, & o->wasm, o->wasmEnd));
    _throwif ("data.drop: data index out of range",
              data_idx >= o->module->numDataSegments);
...
//---------------------------------------------------------------------------------------------------------------------
// Atomic ops (threads proposal, 0xFE prefix)
//---------------------------------------------------------------------------------------------------------------------

// Force any int register operand into a slot, then emit op + N popped slot
// indices + memory offset. `i_numInputs` is how many wasm-stack operands to
// consume (top first); `i_resultType` is c_m3Type_none for stores or the
// result type otherwise (result lands in the int register).
static
M3Result  EmitAtomicMemOp  (IM3Compilation o, m3opcode_t i_opcode,
                            u32 i_numInputs, u8 i_resultType)
{
    M3Result result;
...
_   (ReadLEB_u32 (& memoryOffset, & o->wasm, o->wasmEnd));
    ...
    IM3OpInfo opInfo = GetOpInfo (i_opcode);
    _throwif (m3Err_unknownOpcode, !opInfo);

    IM3Operation op = opInfo->operations [0];
    _throwif (m3Err_noCompiler, !op);

    // Make sure no operand is in a register — atomic ops always read from slots.
_   (PreserveRegisterIfOccupied (o, c_m3Type_i64));

_   (EmitOp (o, op));
    for (u32 i = 0; i < i_numInputs; ++i)
_       (EmitSlotNumOfStackTopAndPop (o));

    EmitConstant32 (o, memoryOffset);

    if (i_resultType != c_m3Type_none)
_       (PushRegister (o, i_resultType));
}
    _catch: return result;
}

M3Result  Compile_Atomic_Load     (IM3Compilation o, m3opcode_t i)
{ return EmitAtomicMemOp (o, i, 1, GetOpInfo (i)->type); }

M3Result  Compile_Atomic_Store    (IM3Compilation o, m3opcode_t i)
{ return EmitAtomicMemOp (o, i, 2, c_m3Type_none); }

M3Result  Compile_Atomic_RMW      (IM3Compilation o, m3opcode_t i)
{ return EmitAtomicMemOp (o, i, 2, GetOpInfo (i)->type); }

M3Result  Compile_Atomic_Cmpxchg  (IM3Compilation o, m3opcode_t i)
{ return EmitAtomicMemOp (o, i, 3, GetOpInfo (i)->type); }

M3Result  Compile_Atomic_Wait     (IM3Compilation o, m3opcode_t i)
{ return EmitAtomicMemOp (o, i, 3, c_m3Type_i32); }

M3Result  Compile_Atomic_Notify   (IM3Compilation o, m3opcode_t i)
{ return EmitAtomicMemOp (o, i, 2, c_m3Type_i32); }

M3Result  Compile_Atomic_Fence    (IM3Compilation o, m3opcode_t i_opcode)
{
    M3Result result;
_try {
    (void) i_opcode;
...
_   (Read_u8 (& reserved, & o->wasm, o->wasmEnd));
    _throwif (m3Err_wasmMalformed, reserved != 0);
_   (EmitOp (o, op_Atomic_Fence));
}
    _catch: return result;
}
...
const M3OpInfo c_operationsFC [] =
{
    ...
    M3OP( "memory.init",           -3,  none,   d_emptyOpList,                           Compile_Memory_Init ),     // 0x08
    M3OP( "data.drop",              0,  none,   d_emptyOpList,                           Compile_Memory_DataDrop ), // 0x09
...
// 0xFE prefix: threads / atomic ops. Only ops we implement are listed; unlisted
// slots stay zero-init (M3OP_RESERVED equivalent) so unknown ones return NULL
// from GetOpInfo and trip m3Err_unknownOpcode.
//
// stackOffset values:
//   load (1->1):  0     store (2->0):  -2
//   rmw  (2->1):  -1    cmpxchg (3->1): -2
//   wait (3->1): -2     notify (2->1):  -1
//   fence(0->0):  0
#define d_atomicOpList(OP)              { op_##OP, NULL, NULL, NULL }

const M3OpInfo c_operationsFE [] =
{
    M3OP( "memory.atomic.notify",    -1, i_32,  d_atomicOpList (Memory_AtomicNotify), Compile_Atomic_Notify ), // 0x00
    M3OP( "memory.atomic.wait32",    -2, i_32,  d_atomicOpList (Memory_AtomicWait32), Compile_Atomic_Wait   ), // 0x01
    M3OP( "memory.atomic.wait64",    -2, i_32,  d_atomicOpList (Memory_AtomicWait64), Compile_Atomic_Wait   ), // 0x02
    M3OP( "atomic.fence",             0, none,  d_atomicOpList (Atomic_Fence),        Compile_Atomic_Fence  ), // 0x03

    M3OP_RESERVED, M3OP_RESERVED, M3OP_RESERVED, M3OP_RESERVED, M3OP_RESERVED,                                // 0x04 - 0x08
    M3OP_RESERVED, M3OP_RESERVED,                                                                              // 0x09 - 0x0a
    M3OP_RESERVED, M3OP_RESERVED, M3OP_RESERVED, M3OP_RESERVED, M3OP_RESERVED,                                // 0x0b - 0x0f

    M3OP( "i32.atomic.load",          0, i_32,  d_atomicOpList (u32_AtomicLoad_u32),  Compile_Atomic_Load   ), // 0x10
    M3OP( "i64.atomic.load",          0, i_64,  d_atomicOpList (u64_AtomicLoad_u64),  Compile_Atomic_Load   ), // 0x11
    M3OP( "i32.atomic.load8_u",       0, i_32,  d_atomicOpList (u32_AtomicLoad_u8),   Compile_Atomic_Load   ), // 0x12
    M3OP( "i32.atomic.load16_u",      0, i_32,  d_atomicOpList (u32_AtomicLoad_u16),  Compile_Atomic_Load   ), // 0x13
    M3OP( "i64.atomic.load8_u",       0, i_64,  d_atomicOpList (u64_AtomicLoad_u8),   Compile_Atomic_Load   ), // 0x14
    M3OP( "i64.atomic.load16_u",      0, i_64,  d_atomicOpList (u64_AtomicLoad_u16),  Compile_Atomic_Load   ), // 0x15
    M3OP( "i64.atomic.load32_u",      0, i_64,  d_atomicOpList (u64_AtomicLoad_u32),  Compile_Atomic_Load   ), // 0x16

    M3OP( "i32.atomic.store",        -2, none,  d_atomicOpList (u32_AtomicStore_u32), Compile_Atomic_Store  ), // 0x17
    M3OP( "i64.atomic.store",        -2, none,  d_atomicOpList (u64_AtomicStore_u64), Compile_Atomic_Store  ), // 0x18
    M3OP( "i32.atomic.store8",       -2, none,  d_atomicOpList (u32_AtomicStore_u8),  Compile_Atomic_Store  ), // 0x19
    M3OP( "i32.atomic.store16",      -2, none,  d_atomicOpList (u32_AtomicStore_u16), Compile_Atomic_Store  ), // 0x1a
    M3OP( "i64.atomic.store8",       -2, none,  d_atomicOpList (u64_AtomicStore_u8),  Compile_Atomic_Store  ), // 0x1b
    M3OP( "i64.atomic.store16",      -2, none,  d_atomicOpList (u64_AtomicStore_u16), Compile_Atomic_Store  ), // 0x1c
    M3OP( "i64.atomic.store32",      -2, none,  d_atomicOpList (u64_AtomicStore_u32), Compile_Atomic_Store  ), // 0x1d

    M3OP( "i32.atomic.rmw.add",      -1, i_32,  d_atomicOpList (u32_AtomicRmwAdd_u32),  Compile_Atomic_RMW ), // 0x1e
    M3OP( "i64.atomic.rmw.add",      -1, i_64,  d_atomicOpList (u64_AtomicRmwAdd_u64),  Compile_Atomic_RMW ), // 0x1f
    M3OP( "i32.atomic.rmw8.add_u",   -1, i_32,  d_atomicOpList (u32_AtomicRmwAdd_u8),   Compile_Atomic_RMW ), // 0x20
    M3OP( "i32.atomic.rmw16.add_u",  -1, i_32,  d_atomicOpList (u32_AtomicRmwAdd_u16),  Compile_Atomic_RMW ), // 0x21
    M3OP( "i64.atomic.rmw8.add_u",   -1, i_64,  d_atomicOpList (u64_AtomicRmwAdd_u8),   Compile_Atomic_RMW ), // 0x22
    M3OP( "i64.atomic.rmw16.add_u",  -1, i_64,  d_atomicOpList (u64_AtomicRmwAdd_u16),  Compile_Atomic_RMW ), // 0x23
    M3OP( "i64.atomic.rmw32.add_u",  -1, i_64,  d_atomicOpList (u64_AtomicRmwAdd_u32),  Compile_Atomic_RMW ), // 0x24

    M3OP( "i32.atomic.rmw.sub",      -1, i_32,  d_atomicOpList (u32_AtomicRmwSub_u32),  Compile_Atomic_RMW ), // 0x25
    M3OP( "i64.atomic.rmw.sub",      -1, i_64,  d_atomicOpList (u64_AtomicRmwSub_u64),  Compile_Atomic_RMW ), // 0x26
    M3OP( "i32.atomic.rmw8.sub_u",   -1, i_32,  d_atomicOpList (u32_AtomicRmwSub_u8),   Compile_Atomic_RMW ), // 0x27
    M3OP( "i32.atomic.rmw16.sub_u",  -1, i_32,  d_atomicOpList (u32_AtomicRmwSub_u16),  Compile_Atomic_RMW ), // 0x28
    M3OP( "i64.atomic.rmw8.sub_u",   -1, i_64,  d_atomicOpList (u64_AtomicRmwSub_u8),   Compile_Atomic_RMW ), // 0x29
    M3OP( "i64.atomic.rmw16.sub_u",  -1, i_64,  d_atomicOpList (u64_AtomicRmwSub_u16),  Compile_Atomic_RMW ), // 0x2a
    M3OP( "i64.atomic.rmw32.sub_u",  -1, i_64,  d_atomicOpList (u64_AtomicRmwSub_u32),  Compile_Atomic_RMW ), // 0x2b

    M3OP( "i32.atomic.rmw.and",      -1, i_32,  d_atomicOpList (u32_AtomicRmwAnd_u32),  Compile_Atomic_RMW ), // 0x2c
    M3OP( "i64.atomic.rmw.and",      -1, i_64,  d_atomicOpList (u64_AtomicRmwAnd_u64),  Compile_Atomic_RMW ), // 0x2d
    M3OP( "i32.atomic.rmw8.and_u",   -1, i_32,  d_atomicOpList (u32_AtomicRmwAnd_u8),   Compile_Atomic_RMW ), // 0x2e
    M3OP( "i32.atomic.rmw16.and_u",  -1, i_32,  d_atomicOpList (u32_AtomicRmwAnd_u16),  Compile_Atomic_RMW ), // 0x2f
    M3OP( "i64.atomic.rmw8.and_u",   -1, i_64,  d_atomicOpList (u64_AtomicRmwAnd_u8),   Compile_Atomic_RMW ), // 0x30
    M3OP( "i64.atomic.rmw16.and_u",  -1, i_64,  d_atomicOpList (u64_AtomicRmwAnd_u16),  Compile_Atomic_RMW ), // 0x31
    M3OP( "i64.atomic.rmw32.and_u",  -1, i_64,  d_atomicOpList (u64_AtomicRmwAnd_u32),  Compile_Atomic_RMW ), // 0x32

    M3OP( "i32.atomic.rmw.or",       -1, i_32,  d_atomicOpList (u32_AtomicRmwOr_u32),   Compile_Atomic_RMW ), // 0x33
    M3OP( "i64.atomic.rmw.or",       -1, i_64,  d_atomicOpList (u64_AtomicRmwOr_u64),   Compile_Atomic_RMW ), // 0x34
    M3OP( "i32.atomic.rmw8.or_u",    -1, i_32,  d_atomicOpList (u32_AtomicRmwOr_u8),    Compile_Atomic_RMW ), // 0x35
    M3OP( "i32.atomic.rmw16.or_u",   -1, i_32,  d_atomicOpList (u32_AtomicRmwOr_u16),   Compile_Atomic_RMW ), // 0x36
    M3OP( "i64.atomic.rmw8.or_u",    -1, i_64,  d_atomicOpList (u64_AtomicRmwOr_u8),    Compile_Atomic_RMW ), // 0x37
    M3OP( "i64.atomic.rmw16.or_u",   -1, i_64,  d_atomicOpList (u64_AtomicRmwOr_u16),   Compile_Atomic_RMW ), // 0x38
    M3OP( "i64.atomic.rmw32.or_u",   -1, i_64,  d_atomicOpList (u64_AtomicRmwOr_u32),   Compile_Atomic_RMW ), // 0x39

    M3OP( "i32.atomic.rmw.xor",      -1, i_32,  d_atomicOpList (u32_AtomicRmwXor_u32),  Compile_Atomic_RMW ), // 0x3a
    M3OP( "i64.atomic.rmw.xor",      -1, i_64,  d_atomicOpList (u64_AtomicRmwXor_u64),  Compile_Atomic_RMW ), // 0x3b
    M3OP( "i32.atomic.rmw8.xor_u",   -1, i_32,  d_atomicOpList (u32_AtomicRmwXor_u8),   Compile_Atomic_RMW ), // 0x3c
    M3OP( "i32.atomic.rmw16.xor_u",  -1, i_32,  d_atomicOpList (u32_AtomicRmwXor_u16),  Compile_Atomic_RMW ), // 0x3d
    M3OP( "i64.atomic.rmw8.xor_u",   -1, i_64,  d_atomicOpList (u64_AtomicRmwXor_u8),   Compile_Atomic_RMW ), // 0x3e
    M3OP( "i64.atomic.rmw16.xor_u",  -1, i_64,  d_atomicOpList (u64_AtomicRmwXor_u16),  Compile_Atomic_RMW ), // 0x3f
    M3OP( "i64.atomic.rmw32.xor_u",  -1, i_64,  d_atomicOpList (u64_AtomicRmwXor_u32),  Compile_Atomic_RMW ), // 0x40

    M3OP( "i32.atomic.rmw.xchg",     -1, i_32,  d_atomicOpList (u32_AtomicRmwXchg_u32), Compile_Atomic_RMW ), // 0x41
    M3OP( "i64.atomic.rmw.xchg",     -1, i_64,  d_atomicOpList (u64_AtomicRmwXchg_u64), Compile_Atomic_RMW ), // 0x42
    M3OP( "i32.atomic.rmw8.xchg_u",  -1, i_32,  d_atomicOpList (u32_AtomicRmwXchg_u8),  Compile_Atomic_RMW ), // 0x43
    M3OP( "i32.atomic.rmw16.xchg_u", -1, i_32,  d_atomicOpList (u32_AtomicRmwXchg_u16), Compile_Atomic_RMW ), // 0x44
    M3OP( "i64.atomic.rmw8.xchg_u",  -1, i_64,  d_atomicOpList (u64_AtomicRmwXchg_u8),  Compile_Atomic_RMW ), // 0x45
    M3OP( "i64.atomic.rmw16.xchg_u", -1, i_64,  d_atomicOpList (u64_AtomicRmwXchg_u16), Compile_Atomic_RMW ), // 0x46
    M3OP( "i64.atomic.rmw32.xchg_u", -1, i_64,  d_atomicOpList (u64_AtomicRmwXchg_u32), Compile_Atomic_RMW ), // 0x47

    M3OP( "i32.atomic.rmw.cmpxchg",      -2, i_32, d_atomicOpList (u32_AtomicCmpxchg_u32), Compile_Atomic_Cmpxchg ), // 0x48
    M3OP( "i64.atomic.rmw.cmpxchg",      -2, i_64, d_atomicOpList (u64_AtomicCmpxchg_u64), Compile_Atomic_Cmpxchg ), // 0x49
    M3OP( "i32.atomic.rmw8.cmpxchg_u",   -2, i_32, d_atomicOpList (u32_AtomicCmpxchg_u8),  Compile_Atomic_Cmpxchg ), // 0x4a
    M3OP( "i32.atomic.rmw16.cmpxchg_u",  -2, i_32, d_atomicOpList (u32_AtomicCmpxchg_u16), Compile_Atomic_Cmpxchg ), // 0x4b
    M3OP( "i64.atomic.rmw8.cmpxchg_u",   -2, i_64, d_atomicOpList (u64_AtomicCmpxchg_u8),  Compile_Atomic_Cmpxchg ), // 0x4c
    M3OP( "i64.atomic.rmw16.cmpxchg_u",  -2, i_64, d_atomicOpList (u64_AtomicCmpxchg_u16), Compile_Atomic_Cmpxchg ), // 0x4d
    M3OP( "i64.atomic.rmw32.cmpxchg_u",  -2, i_64, d_atomicOpList (u64_AtomicCmpxchg_u32), Compile_Atomic_Cmpxchg ), // 0x4e
...
IM3OpInfo  GetOpInfo  (m3opcode_t opcode)
{
    ...
    case 0xFE:
        opcode &= 0xFF;
        if (opcode < M3_COUNT_OF(c_operationsFE)) {
            const M3OpInfo * info = &c_operationsFE[opcode];
            // M3OP_RESERVED entries have a NULL compiler pointer; treat as unknown
...
M3Result  CompileFunction  (IM3Function io_function)
{
    ...
    if (!io_function->wasm) {
        fprintf(stderr, "wasm3: missing body for '%s'",
                m3_GetFunctionName(io_function));
        if (io_function->import.moduleUtf8)
            fprintf(stderr, " (import: %s.%s)", io_function->import.moduleUtf8, io_function->import.fieldUtf8);
        fprintf(stderr, "\n");
