...
typedef struct M3Memory
{
    ...
    // Threads hack: when true, this M3Memory's `mallocated` buffer is owned
    // by a different runtime (the parent) and must NOT be freed or grown here.
    // Set by m3_NewSiblingRuntime.
    bool                    shared;
}
M3Memory;
