...
_   (ReadLEB_u32 (& numDataSegments, & i_bytes, i_end));                            m3log (parse, "** Data [%d]", numDataSegments);
    ...
    for (u32 i = 0; i < numDataSegments; ++i)
    {
        ...
        // Bulk-memory data-segment header. Per the wasm spec:
        //   flag 0 : active (memory 0)         — has init_expr + size + data
        //   flag 1 : passive                   — has size + data only
        //   flag 2 : active (explicit memidx)  — has memidx + init_expr + size + data
        // Older wasm3 only handled flag 0; tools that emit `--shared-memory`
        // wasm produce passive segments and rely on `memory.init` at runtime.
        u32 flag;
_       (ReadLEB_u32 (& flag, & i_bytes, i_end));

        if (flag == 0u) {
            segment->memoryRegion = 0;
...
_           (Parse_InitExpr (io_module, & i_bytes, i_end));
            ...
            // Passive — no offset, initialised at runtime via memory.init.
            segment->memoryRegion  = 0;
...
_           (Parse_InitExpr (io_module, & i_bytes, i_end));
            ...
            _throw ("unknown data segment flag");
        }

_       (ReadLEB_u32 (& segment->size, & i_bytes, i_end));
        segment->data = i_bytes;                                                    m3log (parse, "    segment [%u]  flag: %u;  memory: %u;  expr-size: %d;  size: %d",
                                                                                       i, flag, segment->memoryRegion, segment->initExprSize, segment->size);
